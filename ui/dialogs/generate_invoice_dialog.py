# C:\Users\tyler\Desktop\FoundersSOManager\ui\generate_invoice_dialog.py
# -*- coding: utf-8 -*-
...
class GenerateInvoiceDialog(QDialog):
    def __init__(self, parent=None, repo=None, so_id: Optional[int] = None):
        ...
        self.dateInvoice = QLineEdit()  # stored as ISO text
        ...
        # Notes + randomizer button
        self.txtNotes = QTextEdit(); self.txtNotes.setFixedHeight(60)
        self.btnRandomNote = QPushButton("Random note")
        self.btnRandomNote.setToolTip("Insert a friendly one-liner into Notes. Clicking again replaces the previous auto note.")
        ...
        # Notes row with button in the label slot
        notes_label_row = QWidget()
        nl = QHBoxLayout(notes_label_row); nl.setContentsMargins(0, 0, 0, 0)
        nl.addWidget(QLabel("Notes"))
        nl.addStretch(1)
        nl.addWidget(self.btnRandomNote)

        money = QFormLayout()
        money.addRow("Tax % (0-1)", self.spnTaxPct)
        money.addRow("Discount ($)", self.spnDiscount)
        money.addRow(notes_label_row, self.txtNotes)
        ...
        self.btnRandomNote.clicked.connect(self._inject_random_note)

        # Pool of one-liners. Edit freely.
    ...
    def _inject_random_note(self):
        ...
        self.txtNotes.setPlainText(updated)
